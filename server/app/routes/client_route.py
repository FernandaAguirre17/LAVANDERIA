...
def search_by_name():
    ...
    #En una sola linea 
    #Ciclo generativo, al encerrarlo en corchetes devuelve el arreglo 
...
def update(client_id):
    ...
    if not client:
        return jsonify({"error":"Cliente no encontrado :/"}), 400
    return jsonify({"msg":"Cliente actualizado con exito :3"}), 200
...
def delete(client_id):
    ...
    if not client:
        return jsonify({"error":"Cliente no encontrado :/"}), 400
    return jsonify({"msg":"Cliente eliminado con exito :3"}), 200
