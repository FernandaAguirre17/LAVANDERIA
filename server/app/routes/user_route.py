...
def login():
    ...
    return jsonify({"msg":"Algo salio mal al intentar iniciar sesion, revise sus claves :3 "}), 400

@user_bp.route("/logout/<int:user_id>", methods=["POST"])
def logout(user_id):
    logout_user(user_id)
    return jsonify({"mgs":"Logout exitoso :3"}), 200
...
def update(user_id):
    ...
    return jsonify({"msg":"Algo salio mal al intentar actualizar el usuario :3 "}), 400
...
def change_status(user_id):
    ...
    return jsonify({"msg":"Algo salio mal al actualizar el estado del usuario >:c"}), 400
